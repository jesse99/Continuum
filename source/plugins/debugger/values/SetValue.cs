...
// /usr/local/bin/multimethod --result=void --name=SetValue --arg=VariableItem:item --arg=object:parent --arg=object:key --arg=object:newValue --namespace=Debugger --using=Mono.Debugger.Soft --comment=Updates the value for an item.
...
namespace Debugger
{
	...
	internal static class SetValue
	{
		...
		public static void Invoke(VariableItem item, object parent, object key, object newValue)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(item, parent, key, newValue);
			...
			ms_candidates[__index].Method(item, parent, key, newValue);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static void NextMethod(VariableItem item, object parent, object key, object newValue)
		{
			var __actual = new Entry(item, parent, key, newValue);
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(VariableItem a1, object a2, object a3, object a4) => temp.Invoke(null, new object[]{a1, a2, a3, a4}),
							parms[0].ParameterType, parms[1].ParameterType, parms[2].ParameterType, parms[3].ParameterType);
					}
					else
					{
						entry = new Entry(
							(VariableItem a1, object a2, object a3, object a4) => temp.Invoke(a1, new object[]{a2, a3, a4}),
		...
		private delegate void OverloadType(VariableItem item, object parent, object key, object newValue);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(VariableItem item, object parent, object key, object newValue) : this()
			{
				Method = null;
				Type1 = (object) item != null ? item.GetType() : null;
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			else
			{
				...
				if (!typeof(VariableItem).IsAssignableFrom(info.DeclaringType))
					throw new InvalidOperationException(string.Format("{0}.{1} this argument is not compatible with type VariableItem.", info.DeclaringType.Name, info.Name));
