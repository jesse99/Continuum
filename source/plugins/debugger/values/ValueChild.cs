...
// /usr/local/bin/multimethod --result=VariableItem --name=ValueChild --arg=ThreadMirror:thread --arg=object:owner --arg=object:item --arg=int:index --namespace=Debugger --using=Mono.Debugger.Soft
...
namespace Debugger
{
	...
	internal static class ValueChild
	{
		...
		public static VariableItem Invoke(ThreadMirror thread, object owner, object item, int index)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(thread, owner, item, index);
			...
			return ms_candidates[__index].Method(thread, owner, item, index);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static VariableItem NextMethod(ThreadMirror thread, object owner, object item, int index)
		{
			var __actual = new Entry(thread, owner, item, index);
		...
		private delegate VariableItem OverloadType(ThreadMirror thread, object owner, object item, int index);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ThreadMirror thread, object owner, object item, int index) : this()
			{
				Method = null;
