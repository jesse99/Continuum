...
// /usr/local/bin/multimethod --result=Item --name=GetItem --arg=ThreadMirror:thread --arg=object:hint --arg=object:value --namespace=Debugger --using=Mono.Debugger.Soft --comment=Returns VariableItem information about arbitrary values.
...
namespace Debugger
{
	...
	internal static class GetItem
	{
		...
		public static Item Invoke(ThreadMirror thread, object hint, object value)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(thread, hint, value);
			...
			return ms_candidates[__index].Method(thread, hint, value);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static Item NextMethod(ThreadMirror thread, object hint, object value)
		{
			var __actual = new Entry(thread, hint, value);
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, object a3) => (Item) temp.Invoke(null, new object[]{a1, a2, a3}),
							parms[0].ParameterType, parms[1].ParameterType, parms[2].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, object a3) => (Item) temp.Invoke(a1, new object[]{a2, a3}),
		...
		private delegate Item OverloadType(ThreadMirror thread, object hint, object value);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ThreadMirror thread, object hint, object value) : this()
			{
				Method = null;
				Type1 = (object) thread != null ? thread.GetType() : null;
				Type2 = (object) hint != null ? hint.GetType() : null;
