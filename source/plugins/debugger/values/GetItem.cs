...
// /usr/local/bin/multimethod --result=Item --name=GetItem --arg=ThreadMirror:thread --arg=object:parent --arg=object:key --arg=object:value --namespace=Debugger --using=Mono.Debugger.Soft --comment="Returns VariableItem information about arbitrary values."
...
namespace Debugger
{
	...
	internal static class GetItem
	{
		...
		public static Item Invoke(ThreadMirror thread, object parent, object key, object value)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(thread, parent, key, value);
			...
			return ms_candidates[__index].Method(thread, parent, key, value);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static Item NextMethod(ThreadMirror thread, object parent, object key, object value)
		{
			var __actual = new Entry(thread, parent, key, value);
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, object a3, object a4) => (Item) temp.Invoke(null, new object[]{a1, a2, a3, a4}),
							parms[0].ParameterType, parms[1].ParameterType, parms[2].ParameterType, parms[3].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, object a3, object a4) => (Item) temp.Invoke(a1, new object[]{a2, a3, a4}),
		...
		private delegate Item OverloadType(ThreadMirror thread, object parent, object key, object value);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ThreadMirror thread, object parent, object key, object value) : this()
			{
				Method = null;
				Type1 = (object) thread != null ? thread.GetType() : null;
				Type2 = (object) parent != null ? parent.GetType() : null;
				Type3 = (object) key != null ? key.GetType() : null;
				Type4 = (object) value != null ? value.GetType() : null;
			}
