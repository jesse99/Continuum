...
// /usr/local/bin/multimethod --result=Item --name=GetItem --arg=ThreadMirror:thread --arg=object:value --namespace=Debugger --using=Mono.Debugger.Soft --comment=Returns VariableItem information about arbitrary values.
...
namespace Debugger
{
	// Returns VariableItem information about arbitrary values.
	internal static class GetItem
	{
		...
		public static Item Invoke(ThreadMirror thread, object value)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(thread, value);
			int __index = DoGetNextMethod(__actual, 0);
			if (__index == -1)
				throw new MissingMethodException(string.Format("Invoke failed to find an overload for GetItem({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for GetItem({0}).", __actual));
			
			return ms_candidates[__index].Method(thread, value);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static Item NextMethod(ThreadMirror thread, object value)
		{
			var __actual = new Entry(thread, value);
			
			int __index = ms_nextIndex;
			if (__index == -1)
				throw new MissingMethodException(string.Format("NextMethod failed to find an overload for GetItem({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for GetItem({0}).", __actual));
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ThreadMirror a1, object a2) => (Item) temp.Invoke(null, new object[]{a1, a2}),
							parms[0].ParameterType, parms[1].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ThreadMirror a1, object a2) => (Item) temp.Invoke(a1, new object[]{a2}),
		...
		private delegate Item OverloadType(ThreadMirror thread, object value);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ThreadMirror thread, object value) : this()
			{
				Method = null;
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			if (!typeof(Item).IsAssignableFrom(info.ReturnType))
				throw new InvalidOperationException(string.Format("{0}.{1} has return type {2} but should have Item (or a derived class).", info.DeclaringType.Name, info.Name, info.ReturnType));
