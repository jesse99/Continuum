...
// /usr/local/bin/multimethod --result=VariableItem --name=GetChild --arg=ThreadMirror:thread --arg=object:value --arg=int:index --namespace=Debugger --using=Mono.Debugger.Soft --comment=Returns the nth VariableItem for an arbitrary value.
...
namespace Debugger
{
	// Returns the nth VariableItem for an arbitrary value.
	internal static class GetChild
	{
		...
		public static VariableItem Invoke(ThreadMirror thread, object value, int index)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(thread, value, index);
			...
			return ms_candidates[__index].Method(thread, value, index);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static VariableItem NextMethod(ThreadMirror thread, object value, int index)
		{
			var __actual = new Entry(thread, value, index);
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, int a3) => (VariableItem) temp.Invoke(null, new object[]{a1, a2, a3}),
							parms[0].ParameterType, parms[1].ParameterType, parms[2].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, int a3) => (VariableItem) temp.Invoke(a1, new object[]{a2, a3}),
		...
		private delegate VariableItem OverloadType(ThreadMirror thread, object value, int index);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ThreadMirror thread, object value, int index) : this()
			{
				Method = null;
				Type1 = (object) thread != null ? thread.GetType() : null;
				Type2 = (object) value != null ? value.GetType() : null;
				Type3 = (object) index != null ? index.GetType() : null;
			}
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			else
			{
				...
				if (!typeof(int).IsAssignableFrom(parms[1].ParameterType))
					throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type int.", info.DeclaringType.Name, info.Name, parms[1].Name));
			}
		}
		#endregion
