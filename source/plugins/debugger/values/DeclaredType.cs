...
// /usr/local/bin/multimethod --result=TypeMirror --name=DeclaredType --arg=object:parent --arg=object:key --namespace=Debugger --using=Mono.Debugger.Soft --comment="Returns the declared type for a child."
...
namespace Debugger
{
	// Returns the declared type for a child.
	internal static class DeclaredType
	{
		...
		public static TypeMirror Invoke(object parent, object key)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(parent, key);
			int __index = DoGetNextMethod(__actual, 0);
			if (__index == -1)
				throw new MissingMethodException(string.Format("Invoke failed to find an overload for DeclaredType({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for DeclaredType({0}).", __actual));
			
			return ms_candidates[__index].Method(parent, key);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static TypeMirror NextMethod(object parent, object key)
		{
			var __actual = new Entry(parent, key);
			
			int __index = ms_nextIndex;
			if (__index == -1)
				throw new MissingMethodException(string.Format("NextMethod failed to find an overload for DeclaredType({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for DeclaredType({0}).", __actual));
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(object a1, object a2) => (TypeMirror) temp.Invoke(null, new object[]{a1, a2}),
							parms[0].ParameterType, parms[1].ParameterType);
					}
					else
					{
						entry = new Entry(
							(object a1, object a2) => (TypeMirror) temp.Invoke(a1, new object[]{a2}),
							temp.DeclaringType, parms[0].ParameterType);
					}
		...
		private delegate TypeMirror OverloadType(object parent, object key);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(object parent, object key) : this()
			{
				Method = null;
				Type1 = (object) parent != null ? parent.GetType() : null;
				Type2 = (object) key != null ? key.GetType() : null;
			}
			
			public Entry(OverloadType overload, Type type1, Type type2) : this()
			{
				Method = overload;
				Type1 = type1;
				Type2 = type2;
		...
		private static int DoLeftPrecedesRight(Entry left, Entry right)
		{
			...
			if ((leftPrecedes1 || leftPrecedes2) && !rightPrecedes1 && !rightPrecedes2)
				return -1;
			else if ((rightPrecedes1 || rightPrecedes2) && !leftPrecedes1 && !leftPrecedes2)
				return +1;
			else
				return 0;
		}
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			if (!typeof(TypeMirror).IsAssignableFrom(info.ReturnType))
				throw new InvalidOperationException(string.Format("{0}.{1} has return type {2} but should have TypeMirror (or a derived class).", info.DeclaringType.Name, info.Name, info.ReturnType));
			...
			{
				if (parms.Length != 1)
					throw new InvalidOperationException(string.Format("{0}.{1} has arity {2} but should have arity 1.", info.DeclaringType.Name, info.Name, parms.Length));
