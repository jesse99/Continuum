...
// /usr/local/bin/multimethod --result=string --name=ValueText --arg=ThreadMirror:thread --arg=object:owner --arg=object:item --namespace=Debugger --using=Mono.Debugger.Soft
...
namespace Debugger
{
	// Dynamically dispatches to the best ValueText overload.
	internal static class ValueText
	{
		...
		public static string Invoke(ThreadMirror thread, object owner, object item)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(thread, owner, item);
			int __index = DoGetNextMethod(__actual, 0);
			if (__index == -1)
				throw new MissingMethodException(string.Format("Invoke failed to find an overload for ValueText({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for ValueText({0}).", __actual));
			
			return ms_candidates[__index].Method(thread, owner, item);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static string NextMethod(ThreadMirror thread, object owner, object item)
		{
			var __actual = new Entry(thread, owner, item);
			
			int __index = ms_nextIndex;
			if (__index == -1)
				throw new MissingMethodException(string.Format("NextMethod failed to find an overload for ValueText({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for ValueText({0}).", __actual));
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, object a3) => (string) temp.Invoke(null, new object[]{a1, a2, a3}),
							parms[0].ParameterType, parms[1].ParameterType, parms[2].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ThreadMirror a1, object a2, object a3) => (string) temp.Invoke(a1, new object[]{a2, a3}),
		...
		private delegate string OverloadType(ThreadMirror thread, object owner, object item);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ThreadMirror thread, object owner, object item) : this()
			{
				Method = null;
				Type1 = (object) thread != null ? thread.GetType() : null;
				Type2 = (object) owner != null ? owner.GetType() : null;
				Type3 = (object) item != null ? item.GetType() : null;
			}
