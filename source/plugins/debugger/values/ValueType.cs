...
// /usr/local/bin/multimethod --result=string --name=ValueType --arg=object:owner --arg=object:value --namespace=Debugger
using System;
...
namespace Debugger
{
	// Dynamically dispatches to the best ValueType overload.
	internal static class ValueType
	{
		...
		public static string Invoke(object owner, object value)
		{
			...
			if (__index == -1)
				throw new MissingMethodException(string.Format("Invoke failed to find an overload for ValueType({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for ValueType({0}).", __actual));
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static string NextMethod(object owner, object value)
		{
			var __actual = new Entry(owner, value);
			
			int __index = ms_nextIndex;
			if (__index == -1)
				throw new MissingMethodException(string.Format("NextMethod failed to find an overload for ValueType({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for ValueType({0}).", __actual));
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(object a1, object a2) => (string) temp.Invoke(null, new object[]{a1, a2}),
							parms[0].ParameterType, parms[1].ParameterType);
					}
					else
					{
						entry = new Entry(
							(object a1, object a2) => (string) temp.Invoke(a1, new object[]{a2}),
		...
		private delegate string OverloadType(object owner, object value);
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			if (!typeof(string).IsAssignableFrom(info.ReturnType))
				throw new InvalidOperationException(string.Format("{0}.{1} has return type {2} but should have string (or a derived class).", info.DeclaringType.Name, info.Name, info.ReturnType));
