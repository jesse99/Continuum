...
// /usr/local/bin/multimethod --result=Value --name=ParseValue --arg=ThreadMirror:thread --arg=VariableItem:item --arg=object:value --arg=string:newText --namespace=Debugger --using=Mono.Debugger.Soft --comment=Uses the item/value type to create a new value from a string. Returns null on parse errors.
...
namespace Debugger
{
	// Uses the item/value type to create a new value from a string. Returns null on parse errors.
	internal static class ParseValue
	{
		...
		public static Value Invoke(ThreadMirror thread, VariableItem item, object value, string newText)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(thread, item, value, newText);
			int __index = DoGetNextMethod(__actual, 0);
			if (__index == -1)
				throw new MissingMethodException(string.Format("Invoke failed to find an overload for ParseValue({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for ParseValue({0}).", __actual));
			
			return ms_candidates[__index].Method(thread, item, value, newText);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static Value NextMethod(ThreadMirror thread, VariableItem item, object value, string newText)
		{
			var __actual = new Entry(thread, item, value, newText);
			
			int __index = ms_nextIndex;
			if (__index == -1)
				throw new MissingMethodException(string.Format("NextMethod failed to find an overload for ParseValue({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for ParseValue({0}).", __actual));
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ThreadMirror a1, VariableItem a2, object a3, string a4) => (Value) temp.Invoke(null, new object[]{a1, a2, a3, a4}),
							parms[0].ParameterType, parms[1].ParameterType, parms[2].ParameterType, parms[3].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ThreadMirror a1, VariableItem a2, object a3, string a4) => (Value) temp.Invoke(a1, new object[]{a2, a3, a4}),
		...
		private delegate Value OverloadType(ThreadMirror thread, VariableItem item, object value, string newText);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ThreadMirror thread, VariableItem item, object value, string newText) : this()
			{
				Method = null;
				...
				Type4 = (object) newText != null ? newText.GetType() : null;
			}
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			if (!typeof(Value).IsAssignableFrom(info.ReturnType))
				throw new InvalidOperationException(string.Format("{0}.{1} has return type {2} but should have Value (or a derived class).", info.DeclaringType.Name, info.Name, info.ReturnType));
			...
			if (info.IsStatic)
			{
				...
				if (!typeof(string).IsAssignableFrom(parms[3].ParameterType))
					throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type string.", info.DeclaringType.Name, info.Name, parms[3].Name));
			}
			else
			{
				if (parms.Length != 3)
