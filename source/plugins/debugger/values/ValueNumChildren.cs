...
// /usr/local/bin/multimethod --result=int --name=ValueNumChildren --arg=object:owner --arg=object:value --namespace=Debugger
using System;
...
namespace Debugger
{
	// Dynamically dispatches to the best ValueNumChildren overload.
	internal static class ValueNumChildren
	{
		...
		public static int Invoke(object owner, object value)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(owner, value);
			int __index = DoGetNextMethod(__actual, 0);
			if (__index == -1)
				throw new MissingMethodException(string.Format("Invoke failed to find an overload for ValueNumChildren({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for ValueNumChildren({0}).", __actual));
			
			return ms_candidates[__index].Method(owner, value);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static int NextMethod(object owner, object value)
		{
			var __actual = new Entry(owner, value);
			
			int __index = ms_nextIndex;
			if (__index == -1)
				throw new MissingMethodException(string.Format("NextMethod failed to find an overload for ValueNumChildren({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for ValueNumChildren({0}).", __actual));
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(object a1, object a2) => (int) temp.Invoke(null, new object[]{a1, a2}),
							parms[0].ParameterType, parms[1].ParameterType);
					}
					else
					{
						entry = new Entry(
							(object a1, object a2) => (int) temp.Invoke(a1, new object[]{a2}),
		...
		private delegate int OverloadType(object owner, object value);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(object owner, object value) : this()
			{
				Method = null;
				Type1 = (object) owner != null ? owner.GetType() : null;
				Type2 = (object) value != null ? value.GetType() : null;
			}
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			if (!typeof(int).IsAssignableFrom(info.ReturnType))
				throw new InvalidOperationException(string.Format("{0}.{1} has return type {2} but should have int (or a derived class).", info.DeclaringType.Name, info.Name, info.ReturnType));
